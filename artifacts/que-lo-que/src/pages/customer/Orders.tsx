...
const STATUS_CONFIG: Record<string, { label: string; color: string; step: number }> = {
  pending: { label: "Buscando driver", color: "bg-yellow-400/20 text-yellow-400 border-yellow-400/40", step: 1 },
  accepted: { label: "Driver en camino", color: "bg-blue-400/20 text-blue-400 border-blue-400/40", step: 2 },
  picked_up: { label: "Recogido", color: "bg-purple-400/20 text-purple-400 border-purple-400/40", step: 3 },
  delivered: { label: "Entregado", color: "bg-green-400/20 text-green-400 border-green-400/40", step: 4 },
  cancelled: { label: "Cancelado", color: "bg-red-400/20 text-red-400 border-red-400/40", step: 0 },
};
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="bg-black border-b border-yellow-400/20 px-4 py-4 flex items-center gap-3 sticky top-0 z-10">
        ...
        <h1 className="text-xl font-black text-yellow-400">Mis pedidos</h1>
      </div>
      ...
      <div className="px-4 py-4">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 font-bold">No tienes pedidos todavía</p>
            <Link href="/customer">
              <p className="text-yellow-400 mt-3 font-bold">Pide algo</p>
            </Link>
          </div>
        ) : (
          <div className="space-y-3">
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <p className="text-gray-400 text-xs mt-0.5 flex items-center gap-1">
                          <Clock size={10} />
                          {new Date(order.createdAt).toLocaleDateString("es-DO")} · Pedido #{order.id}
                        </p>
                      </div>
